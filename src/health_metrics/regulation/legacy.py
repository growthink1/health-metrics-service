...
def regulate(s: RegulationSignals) -> tuple[RecType, list[str], dict]:
    ...
    recovery_score = -0.50 * s.hrv_z_3d - 0.30 * s.rhr_z_3d + 0.40 * ((s.sleep_3d_min - 360) / 60)
    ...
    rationale.append(
        f"Mixed signals (recovery {recovery_score:.2f}, strain {s.strain_7d_total / 7:.1f}/d) — conservative"
    )
    return (
        "deficit_conservative",
        rationale,
...
async def compute_regulation_signals(session: AsyncSession, user_id: str, anchor: date) -> RegulationSignals:
    ...
    sleep_mins = [float(r.oura_sleep_duration_min) for r in dm_rows if r.oura_sleep_duration_min is not None]
    sleep_debts = [float(r.whoop_sleep_debt_min) for r in dm_rows if r.whoop_sleep_debt_min is not None]
    strain_total = sum(float(r.whoop_day_strain) for r in strain_rows if r.whoop_day_strain is not None)

    energies = [float(r.subjective_energy) for r in ml_rows if r.subjective_energy is not None]
